...
checkLeaderNatValue bn σ f =
  if activeSlotVal f == maxBound
    ...
    -- bet set above one half otherwise.
      True
    else trace(debugInfo σ (-fromRational σ) c recip_q x) $ case taylorExpCmp 3 recip_q x of
      ABOVE _ _ -> False
      BELOW _ _ -> True
      MaxReached _ -> False
  where
    ...
    debugInfo :: Rational -> FixedPoint -> FixedPoint -> FixedPoint -> FixedPoint -> String
    debugInfo aa bb cc dd ee = show aa ++ ", " ++ show bb ++ ", " ++ show cc ++ ", " ++ show dd ++ ", " ++ show ee
